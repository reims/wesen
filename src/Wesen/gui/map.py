...
class Map(GuiObject):
	...
	def DrawMap(self):
		...
		if len(self.descriptor) > 0:
			valuesPerObject = len(self._descToArray(self.descriptor[0]));
	...
	def _BuildData(self):
		...
		if len(self.descriptor) == 0:
			return;
		num_objects = len(self.descriptor);
		values_per_object = len(self._descToArray(self.descriptor[0]));
		self._data_size = 2**ceil(log(num_objects, 2));
		self._data = nzeros(self._data_size*values_per_object, "f");
		self._indices = {};
		for i,obj in enumerate(self.descriptor):
			_id = obj["id"];
